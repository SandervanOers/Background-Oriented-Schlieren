...
{
	unsigned int NumberOfInputArguments = 19;
	// Checks number of Arguments
    if ( argc != NumberOfInputArguments)
    {
		printf("Too few arguments\n");
        printf("usage: DisplayImage.out <Image_Path> SplineDegree SubsetLength GridLength ShapeFunction PropagationFunction OrderingImages xStart xEnd yStart yEnd NumberOfThreads MaxPixelYVertical Tolerance MinCorrCoeffIG DICNeeded CalibrationNeeded CalculateRefractionIndex\n");
		return -1;
    }
	...
	// Saves Command
	std::ofstream myfile;
	myfile.open (inputvariables.path+"/commandline.txt");
	myfile << "usage: DisplayImage.out <Image_Path> SplineDegree SubsetLength GridLength ShapeFunction PropagationFunction OrderingImages xStart xEnd yStart yEnd NumberOfThreads MaxPixelYVertical Tolerance MinCorrCoeffIG DICNeeded CalibrationNeeded CalculateRefractionIndex\n";
	...
	unsigned int DICneeded = atoi(argv[16]);
	...
	unsigned int Calibrationneeded = atoi(argv[17]);
	...
	unsigned int Refractionneeded = atoi(argv[18]);
...
extern int checkinput(unsigned int argc, char *argv[], std::string &pathname_string, unsigned int &SplineDegree, unsigned int &SubsetLength, unsigned int &GridLength, unsigned int &ShapeFunction, unsigned int &propagationfunction, unsigned int &ordering, unsigned int &xStart, unsigned int &xEnd, unsigned int &yStart, unsigned int &yEnd, unsigned int &offset, unsigned int &Number_Of_Threads, unsigned int &MaxPixelYVertical, double &abs_tolerance_threshold, double &rel_tolerance_threshold, double &minimum_corrcoeff_IG )
{
	...
	// Minimum Acceptable Correlation Coefficient for Initial Guess
	minimum_corrcoeff_IG =  atof(argv[15]);
