...
int main(int argc, char** argv )
{
    ...
    if ( argc != 14)
    {
        printf("usage: DisplayImage.out <Image_Path> SplineDegree SubsetLength GridLength ShapeFunction PropagationFunction OrderingImages xStart xEnd yStart yEnd NumberOfThreads MaxPixelYVertical\n");
        return -1;
    }
   ...
   // unsigned int very = yEnd - yStart;
	...
	std::vector<cv::Mat> IG = calculateInitialGuess_Iteration(img, img1, fptr_img1, SplineDegree, SubsetLength, GridLength, horx_ROI, very_ROI, offset, Number_Of_Threads, MaxPixelYVertical, abs_tolerance_threshold, rel_tolerance_threshold, ShapeFunction);
	...
	for (unsigned int i = 0; i<GridX.cols; i++)
	{
   ...
   while(cv::sum(Computed_Points).val[0] < Computed_Points.total())
    {			
		/*
		unsigned int size_list = Locations_Best_Correlation.size();
		//std::cout << "size =  " << std::min(Number_Of_Threads, size_list) <<std::endl;
		
		for (unsigned int l = 0; l < std::min(Number_Of_Threads, size_list); l++) //
		{
			//unsigned int l = 0;
			cv::Point matchLoc = Locations_Best_Correlation[l].Loc;
			...
			//std::vector<std::vector<double>> Solution_Neighbours;
			//unsigned int number_neighbour = 0;
			for (auto i = Neighbours.begin(); i < Neighbours.end(); i++)
			{
				...
				//std::cout << *i << std::endl;
				//threads.push_back(std::thread(iteration_thread,std::ref(Solution_Neighbours), number_neighbour, img, fptr_img1, (*i).x, (*i).y, InitialCondition, SplineDegree, SubsetLength, GridLength, abs_tolerance_threshold, rel_tolerance_threshold, ShapeFunction));			
				//iteration_thread(std::ref(Solution_Neighbours), number_neighbour, img, fptr_img1, (*i).x, (*i).y, InitialCondition, SplineDegree, SubsetLength, GridLength, abs_tolerance_threshold, rel_tolerance_threshold, ShapeFunction);		
				threads.push_back(std::thread(iteration_Thread,img, fptr_img1, (*i).x, (*i).y, InitialCondition, SplineDegree, SubsetLength, GridLength, abs_tolerance_threshold, rel_tolerance_threshold, ShapeFunction, std::ref(DispX), std::ref(DispY), std::ref(Ux), std::ref(Vx), std::ref(Uy), std::ref(Vy), std::ref(Uxy), std::ref(Vxy), std::ref(Uxx), std::ref(Vxx), std::ref(Uyy), std::ref(Vyy), std::ref(CorrelationCoefficient), std::ref(Computed_Points), std::ref(Locations_Best_Correlation)));
				//iteration_Thread(img, fptr_img1, (*i).x, (*i).y, InitialCondition, SplineDegree, SubsetLength, GridLength, abs_tolerance_threshold, rel_tolerance_threshold, ShapeFunction, std::ref(DispX), std::ref(DispY), std::ref(Ux), std::ref(Vx), std::ref(Uy), std::ref(Vy), std::ref(Uxy), std::ref(Vxy), std::ref(Uxx), std::ref(Vxx), std::ref(Uyy), std::ref(Vyy), std::ref(CorrelationCoefficient), std::ref(Computed_Points), std::ref(Locations_Best_Correlation));
				//number_neighbour++;
			}
		for (auto& th : threads) 
			th.join();			
		*/
		...
		//for (int i = 0; i <Solution_Neighbours.size(); i++)
		//{
		//	for (int j = 0; j < Solution_Neighbours[i].size(); j++)
		//	{
		//		std::cout << Solution_Neighbours[i][j] << " ";
		//	}
		//	std::cout << std::endl;
		//}
		/*
		...
		for (auto i = Neighbours.begin(); i < Neighbours.end(); i ++)
		{
			std::vector<double> point2 = Solution_Neighbours
