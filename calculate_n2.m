...
I = abs(Ux)>0.65;%[];%(abs(Uxx)>1e-3 | abs(Ux)>0.1) || CorrCoeff < 0.9); %[];%
...
%                 IC = [-1031, 0.621, 1007];
        ...
        lb = [-inf, .1, -inf];
        ub = [+inf, 10, +inf];
        dx = @(Vx)nfunctiontominimize_Snell2(Vx,DeltaX(:), DeltaY(:), GridX(:), GridY(:), Lengths, Distance_to_mm, focal_length, Xcameracenter, Ycameracenter, take_y_into_account);
...
%  No Filter        Vx = 1e3*[-1.013342820523192   0.000618906447396   1.231240881310953];
%Filter  
% Vx = 1e3*[-1.012686754033436   0.000618796015106 1.230886561404477 ];
%          Vx=1e3*[-1.010368745888901   0.000619561861516   1.231244637819569];
% Filter and abs(Ux) > 0.65
% Vx = 1e3*[ -1.011443499026947   0.000618592449317   1.232042581492979];
Vx = 1e3*[-1.011636197173333   0.000618629182216   1.233159984273969];
% Vx = 1e3*[   -1.010069772686775   0.000619510609764   1.225000000000000]
% Vx = 1e3*[   -1.010370816060937   0.000619561885807   1.230155724224944];
%          display('Determined Optical Axis');
%          Vx=1e2*[-9.308564201116138   0.005654077309783];
% Vx = 1e2*[-9.895948444794531   0.006132287216026];
%     Vx = 1e2*[-9.895948444794531   0.006132287216026];

% Y-depen partial
% Vx = 1e3*[-0.989587747611256   0.000613227416757   1.222755313476285];
% % 
% Vx = 1e3*[-0.989591166902885   0.000613228051278   1.223720290526554];
% % beta=-beta
% Vx = 1e3*[-0.989891338094724   0.000613286064920   1.223420119035178]; % var(n) = 2.994393032943708e-07
% % beta=beta;
% Vx = 1e3*[ -0.989891568300011   0.000613286143339   1.223496447301527]; % 2.996012285418968e-07
% % Vx = 1e3*[-0.986800393011165   0.000612718528115   1.225000000000000];
% Vx=1e3*[  -0.986195486874736   0.000612596939985   1.225000000000000];
% 
% % New
% Vx = 1e3*[  -1.016726832024663   0.000618309814877   1.225000000000000]; % stepsize 500
% Vx = 1e3*[-1.015797586703896   0.000618155997985   1.225000000000000]; % stepsize 100

% Vx=1e3*[ -1.031329513216509   0.000620854664420   1.007432156511911];
% Vx=1e3*[-1.016123525799769   0.000618212697987   1.223745613491268];
% Vx=1e3*[ -1.016804651960030   0.000618347122778   1.220288881977917];
% Vx=1e3*[-1.014745391900410   0.000617992085774   1.225000000000000];
            
% Vx = 1e3*[ -1.016126702399341   0.000618213299700   1.225000000000000];
        % True Values
...
%     [phi_x_1_2_new, phi_y_1_2_new, phi_x_2_2_new, phi_y_2_2_new1] = SnellsLaw(phi_x_1_1_new, phi_y_1_1_new, phi_x_2_1_new, phi_y_2_1_new, alpha, n_glass, 1.333);
    ...
    Asol = (L_camera+L_screen)* (tand(phi_x_1_0)./(cosd(alpha)-tand(phi_x_1_0)*sind(alpha))./(cosd(beta)-tand(phi_y_1_0)*sind(beta))-tand(phi_x_2_0)./(cosd(alpha)-tand(phi_x_2_0)*sind(alpha))./(cosd(beta)-tand(phi_y_2_0)*sind(beta))) ...
        + 2*L_glass * (tand(phi_x_1_1)./(cosd(alpha)-tand(phi_x_1_1)*sind(alpha))./(cosd(beta)-tand(phi_y_1_1)*sind(beta))-tand(phi_x_2_1)./(cosd(alpha)-tand(phi_x_2_1)*sind(alpha))./(cosd(beta)-tand(phi_y_2_1)*sind(beta))) ...
        + L_test *tand(phi_x_1_2)./(cosd(alpha)-tand(phi_x_1_2)*sind(alpha))./(cosd(beta)-tand(phi_y_1_2)*sind(beta));
...
%     n2 = zeros(floor(length(B)/stepsize),1);
    ...
    for i=1:stepsize:length(B)
        ...
        [Vn]  = lsqnonlin(dn,ICC, lbb, ubb,lsqOpts);
         if(flag_halfway_done == 0 && i/length(B)-0.5 > 0) 
...
figure; 
hold on
plot(wmeans,linspace(min(GridY), max(GridY), length(wmeans) )); %(max(GridY)-min(GridY))/3+1)
...
% Tan
T = 16;
lhs = wmeans-1.3373+1.3531e-4*T+5.1e-8*T^2;
